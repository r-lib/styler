...
* the hook revisions in your `.pre-commit-config.yaml` with
  ...
  package updates (exception: Patch releases for hooks).
...
* the upstream pre-commit framework. Use the update utilities provided by your
  ...
  `precommit::version_precommit()`. Updates to the pre-commit framework are 
  not released in sync with the R or hook revision updates.
