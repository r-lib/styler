...
either manualy or with `reticulate::install_miniconda()`. The rest can be 
handled from R:
...
`git commit` are specified. If you want to use a custom 
`.pre-commit-config.yaml` you can set the R option 
`precommit.path_cp_config_from` to the path where this config file is stored, 
like this:
