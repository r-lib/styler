...
* `style-files`: A hook to style files with 
  ...
  the tidyverse style guide. NOTE: devel version of styler strongly suggested. 
  Install via `remotes::install_github('r-lib/styler')`.
* `readme-rmd-rendered`: Make sure `README.Rmd` hasn't been edited 
  more recently than `README.md`, i.e. remind you to render the `.Rmd` to `.md`
  before committing.
...
* `use-tidy-description`: A hook to run `usethis::use_tidy_description()`
  to ensure dependencies are ordered alphabetically and fields are in standard 
  order.
...
* No need to worry about dependencies, testing, different versions 
