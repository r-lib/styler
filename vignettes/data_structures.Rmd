...
(parse_table <- styler:::compute_parse_data_flat_with_ws(code))
```
...
```{r}
styler:::serialize_parse_data(parse_table)
```
...
the styled text as one string (or one string per line) is subject to future work, so would be the functions to be
