...
The tibble contains the column `leaves`, which itself contains a tibble. 
If we "enter"  the first leaf, we can see that the expression was split up 
further.

```{r}
l1$leaves[[1]] %>%
  select(text, terminal, leaves, token)
```

And further...
```{r}
l1$leaves[[1]]$leaves[[3]]$leaves[[5]]$leaves[[2]]
```

... and so on. Every leaf that is not a terminal contains another tibble where 
...
serialize <- function(x) {
  out <- Map(
    ...
    x$terminal, x$token,
    if (is.null(x$leaves)) rep(list(NULL), length(x$terminal)) else x$leaves
  )
  out
}
