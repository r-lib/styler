...
```{r}
styler:::create_filler
```

```{r}
code <- "a <- function(x) { if(x > 1) { 1+1 } else {x} }"
pd_nested <- styler:::compute_parse_data_nested(code)
pd_nested_enhanced <- styler:::visit(pd_nested, c(styler:::create_filler))
pd_nested_enhanced
```
...
```{r}
styler:::visit(pd_nested, 
               c(styler:::create_filler, 
                 purrr::partial(styler:::indent_round, indent_by = 2)))
```
