...
```
  id: style-files
  args: [--style_pkg=<styler>, '--style_transformers=<tidyverse_style(scope = "tokens")'>, --style_fun=<tidyverse_style>]
```

Note that not all combinations of quotes will be parsed correctly, e.g. 
reversing inner and outer quotes won't work and escaping quotes may also not 
work.
