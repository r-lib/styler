...
can silence it with `--no-warn-cache`.

This hook modifies files unless you specify `--dry=fail` (requires 
`{styler} > 1.3.2`), which implies the hook failed if applying styler *would* 
change any code.
...
exists, the hook emits a warning which you can silence it with 
`--no-warn-cache`.
```
  id: royxgenize
  args: [--no-warn-cache] 
