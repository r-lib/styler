...
* `spell-check`: Checks spelling with `spelling::spell_check_files()`. Excluded
  are R and python scrips as well as .gitignore. To customize, see *Hooks
  arguments* below.
...
* `codemeta-description-updated`: Make sure `DESCRIPTION` hasn't been edited more recently
  than `codemeta.json`, i.e. remind you to run `codemetar::write_codemeta()` in order to keep `codemeta.json` in sync with `DESCRIPTION`.
