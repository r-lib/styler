...
Checks spelling with `spelling::spell_check_files()`. Excluded are a range of 
files such `.R` files, files in `data/` etc. As of 
...
extract_spell_check_regex <- function() {
  yaml::read_yaml(system.file("pre-commit-hooks.yaml", package = "precommit")) %>%
...
If you provide an `exclude:` key in your `.pre-commit-config.yaml` under the 
`spell-check` hook, you can specify a regex that will overscope the default
regex from above, also see section *Other hook settings* above. The `lang` 
arg will be passed to `spelling::spell_check_files()`.
