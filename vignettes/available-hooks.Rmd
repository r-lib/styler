...
`style_transformers` if you want to use another style guide than the tidyverse
style guide. Argument `style_fun` is deprecated as of version `0.1.2.9001` and 
will be removed in a future release because `style_transformers` is more 
flexible, e.g. you can specify scope and other arguments of the style guide, 
e.g. `'--style_transformers="tidyverse_style(scope='spaces')". 

```
  id: style-files
  args: [--style_pkg=<styler>, --style_transformers=<tidyverse_style()>, --style_fun=<tidyverse_style>]
```

Note that not all combinations of quotes will be parsed correctly, e.g. 
`'--style_transformers=tidyverse_style(scope = "none")'` works (you can also 
drop the outer quotes), but other combinations of single, double and escaped 
versions of these quoting characters may not work.
