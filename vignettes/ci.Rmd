...
- Limitations for the roxygenize hook:
  ...
  - There is a [timeout of 120s for building a hook environment](https://github.com/pre-commit-ci/runner-image/issues/93#issuecomment-922505301), 
    which might not be enough to build the hook environment for the roxygenize 
    hook (if you package has many dependencies)
...
You can skip a hook on [pre-commit.ci](https://pre-commit.ci/) by setting 
the respective option in `.pre-commit-config.yaml`:
