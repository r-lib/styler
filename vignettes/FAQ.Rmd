...
`{precommit}` uses. Since pre-commit will do all the work for you and sometimes
even caches results for speed, we don't recommend running these things manually.
If you need to know which package versions are used, look for the `renv.lock`
file in https://github.com/lorenzwalthert/precommit at the tag specified under 
