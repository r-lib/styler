...
This vignette provides a high-level overview of how styler works and how you
...
via the `style` argument. The transformers are a named list of transformer 
functions and other arguments passed to styler. To use the default style guide 
of styler ([the tidyverse style guide](http://style.tidyverse.org/)), call 
...
which defaults to `style(...)`. This allows for the styling options to be 
set without having to specify them inside the function passed to `transformers`.
...
`col2`, `line1` and `line2` in the parse table in any of your functions since 
these columns only reflect the position of tokens at the point of parsing, 
...
position of line breaks and spaces. Let's first create the nested parse table.
