...
- through the [GitHub Action
  Workflow](https://github.com/r-lib/actions/tree/v2-branch/examples#style-package)
  that triggers
  [`styler::style_pkg()`](https://styler.r-lib.org/dev/reference/style_pkg.md)
  on changes in source files. Setting this up is easiest with
...
- as a formatter for R Markdown without modifying the source. This
  ...
  `tidy = "styler"` in the header of a code chunks
  (e.g.```` ```{r name-of-the-chunk, tidy = "styler"} ````), or
  `knitr::opts_chunk$set(tidy = "styler")` at the top of your RMarkdown
  script.

- via the [R language
  server](https://github.com/REditorSupport/languageserver) to format
  your code in VS Code, atom and others.
...
- As a [Jupyterlab code
  formatter](https://jupyterlab-code-formatter.readthedocs.io).

- for pretty-printing [drake](https://github.com/ropensci/drake)
  workflow data frames with `drake::drake_plan_source()`.

Do you know another way to use styler that is not listed here? Please
let us know by [opening an
issue](https://github.com/r-lib/styler/issues) and we’ll extend the
list.
